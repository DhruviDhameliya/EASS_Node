...
wss.on("connection", (ws, req) => {
    ...
    if (req.url.slice(1, 2) == "s") {
        ...
        if (isNew) {
            ...
            usercon[uid].push(ws);
            console.log("usercon[uid] : ", usercon[uid].length);
        }
    } else {
        ...
        if (isNew) {
            ...
            console.log("customercon[uid] : ", customercon[uid].length);
        }
    }
    ...
    ws.on("message", async (data) => {
        ...
        console.log("grgrghe", JSON.parse(data));
...
async function channelHandler(msg) {
    if (msg.channel == "SEND_MESSAGE") {
        ...
        // msg.data = { ...msg.data, receiver_id: msg.data.sender_id };
        if (msg.data.c_type == 0) {
            await sendWsMessage(msg.data.receiver_id, msg);
            let staffchat = await adminapi.getChatUsersListSocket(msg.data);
            ...
            await sendMessageToCustomer(msg.data.receiver_id, msg);

            // Send Message To Group Users Except Sender User
            let sql = `SELECT * FROM chat_group_members WHERE group_id = ${msg.data.receiver_id}`;
            await new Promise((resolve, reject) => {
                con.query(sql, function (err, result) {
                    if (err) {
                        ...
                        if (result.length > 0) {
                            // console.log(result, "tiniiiiiiiiiiii");
                            for (let index = 0; index < result.length; index++) {
                                const singleGroupMember = result[index];
                                if (msg.data.sender_id != singleGroupMember.m_u_id) {
                                    let respdata1 = {
                                        channel: "SEND_MESSAGE",
                                        data: { ...msg.data, created_timestamp: moment().format("YYYY-MM-DD HH:mm:ss"), chat_user_type: singleGroupMember.chat_user_type },
                                    };
                                    sendWsMessage(singleGroupMember.m_u_id, respdata1);
                                }
                                // sendWsMessage(singleGroupMember.m_u_id, respdata);
            ...
            sendWsMessage(uid, respdata);

        } else {
            sendWsMessage(msg.data.receiver_id, msg);
        }
    } else if (msg.channel == "READ_CHAT") {
        console.log(msg.data, "msg.data11111111111111111111111");

        await adminapi.readchat(msg.data);


        let staffchat = await adminapi.getChatUsersListSocket(msg.data);
        let uid = { uid: msg.data.login_id };
        let customer = await adminapi.getChatCustomerListSocket(uid);
        let respdata = {
            channel: "READ_CHAT",
            data: {
        ...
        await adminapi.readchat(msg.data);
        let groupList = await adminapi.getChatCustomerListSocket({ uid: msg.data.login_id });
        let respdata = {
            channel: "READ_CHAT_GROUP",
            data: {
                groupList: groupList.data,
        ...
        console.log("ahahahahhahahahahahhahahhhhhhhhhhhhhh");
        let staffchat = await adminapi.getChatUsersListSocket(msg.data);
        ...
        sendWsMessage(msg.data.u_id, respdata);
        // } else {
        //   // let groupList = await adminapi.getChatCustomerListSocket(msg.data.login_id);
        //   // let respdata = {
        //   //   channel: "READ_CHAT_GROUP",
        //   //   data: {
        //   //     groupList: groupList.data,
        ...
        if (msg.data.c_type == 1) {
                        ...
                        if (result.length > 0) {
                            ...
                            for (let index = 0; index < result.length; index++) {
                                ...
                                if (msg.data.login_id != singleGroupMember.m_u_id) {
                                    const tdata = await adminapi.chatnotification({ u_id: singleGroupMember.m_u_id, chat_user_type: result[index].chat_user_type });
                                    let respdata1 = {
                                    ...
                                    console.log(singleGroupMember.m_u_id, "^^^^^^^^^^", respdata1);
                                    sendWsMessage(singleGroupMember.m_u_id, respdata1);
                                }
                                // sendWsMessage(singleGroupMember.m_u_id, respdata);
            ...
            const tdata = await adminapi.chatnotification(msg.data);
            let respdata = {
                channel: "GET_CHAT_NOTIFICATION",
                data: {
                    counter: tdata.counter,
