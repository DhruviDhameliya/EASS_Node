...
const fs = require("fs");
const { getbranchdatabyid } = require("./auth");

module.exports = (invoicedata, generalsetting, customer) => {
  ...
  const invoicefunc = () => {
    ...
    invoicedata.map(async (l, index) => {
      ...
      if (
        fs.existsSync(
          `./public/Assets/images/invoicesignature/${l.customerdata.invoice_signature}`
        )
      ) {
      ...
      idata =
        ...
        <tr style="width:100% ;" >
          ...
          <div class="flex"> 
          <div>
                  <img width="100px"  src=${`data:image/jpg;base64,${imageAsBase64}`} alt="fed" />
                        </div>
                        <div class="mx-2" style="width:80%"> 
                          <div class="flex space-between" style="width:100%">
                            <div>
                              <h3 class="bold">${l.settingdata.c_name} </h3>
                              </div>
                              <div class="fsize">
                                <label class="fsize">Branch :</label> ${
                                  l.customerdata.bname
                                }
                              </div>
                            </div>
                            <div style="word-break: break-word;" class="fsize">${
                              l.settingdata.address
                            } </div>
                            ${
                              l.customerdata.c_tax && l.customerdata.c_tax == 1
                                ? `<div class="fsize">
                                  <label class="fsize">GST No : </label>
                                  ${l.settingdata.c_gstno}
                                </div>`
                                : ``
                            }
                            ${
                              l.customerdata.c_tax && l.customerdata.c_tax == 1
                                ...
                                <label class="fsize">State Name : </label> ${
                                  l.settingdata.state_name
                                    ? l.settingdata.state_name
                                    : " "
                                }, 
                                <label class="fsize">Code : </label> ${
                                  l.settingdata.TIN_number
                                    ? l.settingdata.TIN_number
                                    : ""
                                }
                                </div>`
                      ...
                      ${
                        l.userdata.mobile != null
                          ? `<div class="fsize">
                        <label class="fsize">Contact No : </label>${l.userdata.mobile}</div>`
                          : ``
                      }
                      ${
                        l.userdata.u_email != null
                          ? `<div class="fsize">
                        <label class="fsize">Email : </label>${l.userdata.u_email}<label class="fsize"> Website : </label>
                        <a href='${l.settingdata.website_url}'>${l.settingdata.website_url}</a>
                      </div>`
                          : ``
                      }
                        </div>
          ...
          <th colspan="8" align="center">
            <h5 class="bold" style="margin-bottom:2;margin-top:2;">${
              l.customerdata.c_tax == 1 ? "ORIGINAL TAX INVOICE" : "INVOICE"
            }</h5>
          </th>
        </tr>
    ...
    <tr style="lineHeight: 23px ">
      <td class="text-left" style="width:50% ; padding:6px ;wordBreak:break-word; vertical-align: top;" >
        <div class="block">
          ...
          ${l.customerdata.c_date}</b>
          </div>
          <div class="block">
            <label><b>Invoice No. : </b></label> 
            ${l.paymentres.invoice_no}</div>
            </td>
            <td class="text-right" style="padding: 6px;width:50%; wordBreak:break-word ;vertical-align: top;">
        <div class="block">
          <label><b>Track Id : </b></label>
          ${l.customerdata.tracking_id}
        </div>
        <div class="block">
          <label><b>Apply For : </b></lable> 
          ${(() => {
            if (l.customerdata.c_s_type == 10) {
              return `Attestation`;
            } else if (l.customerdata.c_s_type == 8) {
              return `Visa`;
            } else if (l.customerdata.c_s_type == 20) {
              return `Translation`;
            } else {
              return `Apostille`;
            }
    ...
    <td class="right" style="padding:6px ;width:50%;wordBreak:break-word; vertical-align: top;">
      ...
      ${
        l.customerdata.c_gst == 0
          ? `${
              l.customerdata.f_z_type == 1
                ? `<lable>Free Zone :</lable>`
                : `<lable>ARN No :</lable>`
            } ${
              l.customerdata.f_z_type == 1
                ? "Special Economic Zone"
                : l.customerdata.arn_no
            }`
          : ""
      }
      <div class="block">
        <label>Name :</lable> ${l.customerdata.c_prefix} ${
          l.customerdata.c_name
        }
      </div>
      ${
        l.customerdata.c_tax == 1 && l.customerdata.f_z_type != 2
          ? `<div class="block">
          <label>GST No : </label>${l.customerdata.c_gst_no}
        </div>`
          : `<div class="block">
          <label>Mobile No : </label>${l.customerdata.c_mo_no}
        </div>`
      }
      ${
        l.customerdata.c_tax == 1 && l.customerdata.f_z_type != 2
          ...
          <label>State Name : </label>${
            l.customerdata.state ? l.customerdata.state : " "
          }, 
          <label>Code : </label>${
            l.customerdata.TIN_number ? l.customerdata.TIN_number : ""
          }
        </div>`
          : `<div class="block">
          <label>Email : </label>${l.customerdata.c_email}
        </div>`
      }
      <div class="block">
          <label>Address :</lable> <span style="word-break: break-word;
          white-space: pre-wrap;">${l.customerdata.c_address}</span></div>
    </td>
    <td class="right" style="padding:6px;width:50%;wordBreak:break-word; vertical-align: top;">
      <div style= "marginBottom: 3px">
        <b>Ship To</b>
      ...
      <div class="block">
      <label>Name :</lable> ${l.customerdata.c_prefix} ${l.customerdata.c_name}
    </div>
    <div class="block">
    <label>Mobile No : </label>${l.customerdata.c_mo_no}
  </div>
...
<div class="block">
<label>Address :</lable><span style="word-break: break-word;
white-space: pre-wrap;"> ${l.customerdata.d_address}</span></div>
    </td>
  </tr>
</table>
<table class="details_table" border="1" style="border-collapse: collapse;  margin-top:0px !important">
    ...
    <th class="tdmarginalign">
      <strong>Amount</strong>
          ...
          ${getrow(l.customerdata.c_id)}
          <tr>
      <td colspan="4" class="text-left" style="vertical-align: top;">
        <div>
          <b style= "text-transform: capitalize">
            Total In Words : ${converter.toWords(
              convertamt(l.paymentres.net_amt)
            )}
          </b>
          <br />
          <br />${(() => {
            if (l.customerdata.c_tax == 1) {
              return `<b>Bank Details :</b>
              ${
                l.settingdata.bank_details != null
                  ? l.settingdata.bank_details
                  : ""
              }`;
            } else {
              return "";
      ...
      <td colspan="4"  style="vertical-align: top;">
        <div class="block total_class" >
          ...
          <span class="float_right" style="wordBreak:keep-all;">
            ${convertamt(l.paymentres.total_amount)}
          </span>
        </div>
        ${
          l.paymentres.exp_charge != 0
            ? `<div class="block total_class">
              <label ><b>Extra Charges</b></label>
              <span class="float_right" style="wordBreak:keep-all;">${convertamt(
                l.paymentres.exp_charge
              )}</span>
            </div>`
            : ""
        }
          ${
            l.paymentres.courier != 0
              ? `<div class="block total_class">
                <label ><b>Courier Charge</b></label>
                <span class="float_right" style="wordBreak:keep-all;">${convertamt(
                  l.paymentres.courier
                )}</span>
              </div>`
              : ""
          }
          ${
            l.paymentres.dis_amt != 0
              ? `<div class="block total_class">
                <label ><b>Discount(-)</b></label>
                <span class="float_right" style="wordBreak:keep-all;">${convertamt(
                  l.paymentres.dis_amt
                )}</span>
              </div>`
              : ""
          }
          ${
            l.paymentres.coupon_amt != 0
              ? `<div class="block total_class">
                <label ><b>Coupon Discount(-)</b></label>
                <span class="float_right" style="wordBreak:keep-all;">${convertamt(
                  l.paymentres.coupon_amt
                )}</span>
              </div>`
              : ""
          }
          ...
          <div class="block total_class">
            ...
            <span class="float_right" style="wordBreak:keep-all;">
              ${convertamt(
                parseFloat(l.paymentres.total_amount) +
                  parseFloat(l.paymentres.courier) +
                  parseFloat(l.paymentres.exp_charge) -
                  parseFloat(l.paymentres.dis_amt) -
                  parseFloat(l.paymentres.coupon_amt)
              )}
            </span>
          </div>
          
          ${
            l.customerdata.c_tax == 1
              ? `
                ${
                  l.customerdata.b_state_id == l.paymentres.c_state
                    ? `<div class="block total_class">
                    <label >
          ...
          ${
            l.customerdata.c_gst == 0 && l.customerdata.f_z_type != 0
              ? `<div class="block total_class">
              <label>
                <b>${
                  l.customerdata.f_z_type == 1
          ...
          ${
            l.paymentres.embassy_fees != 0
              ? `<div class="block total_class">
          <label><b>Embassy Fees</b></label>
          <span class="float_right"  style="wordBreak:keep-all;">${convertamt(
            l.paymentres.embassy_fees
          )}</span>
        </div>
    `
              : ""
      ...
      <td class="right" colspan="4" style="width:65%; wordBreak:break-word;vertical-align: top;">
        <div>
          ...
          <p class="fsize">${
            l.settingdata.c_term_condition != null
              ? l.settingdata.c_term_condition
              : ""
          }
            </p>
        </div>
      ...
      <td colspan="4" style="vertical-align: top;">
        <div class="sig">
          <p class="signarureclass">For ${l.settingdata.c_name},</p>
         <!---<p class="signarurestaffclass">For ${
           l.customerdata.cname
         }</p> ---!>
         <img
  ...
  const getrow = (c_id) => {
    let rows = "";
    invoicedata.map((l, index) => {
      if (l.customerdata.c_id == c_id) {
        if (l.customerdata.c_s_type == 10) {
          l.list &&
            l.list.map((adata, index) => {
              let cerficates = "";
              ...
              <td class="tdmargin">${l.settingdata.hsn_code}</td>
              <td class="tdmarginalign">${
                adata.a_certificate.split(",").length
              }</td>
              <td class="tdmarginalign">${adata.amount}</td>
            
              <td class="tdmarginalign">${
                adata.amount * adata.a_certificate.split(",").length
              }</td>
                  </tr>`;
            });
        } else if (l.customerdata.c_s_type == 8) {
          l.list &&
            l.list.map((vdata, index) => {
              let cerficates = "";
              ...
              rows =
               ...
               <td class="tdmarginalign">${vdata.amount}</td>
               <td class="tdmarginalign">${vdata.amount}</td>
                      </tr>`;
            });
        } else if (l.customerdata.c_s_type == 17) {
          l.list &&
            l.list.map((adata, index) => {
              let cerficates = "";
              adata.cerficates != null &&
